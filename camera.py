...
import time
from helpers import get_top3, get_word_suggestions, translate_to_ukrainian, put_unicode_text
    ...
    while cap.isOpened():
        ...
        if results.hand_landmarks:
            ...
            for hand_landmarks in results.hand_landmarks:
                ...
                if model_input_img.size != 0 and (time.time() - last_prediction_time > 2):
                    ...
                    top3 = get_top3(pred, CLASS_NAMES)
        ...
        # # Word suggestions (above subtitle bar)
        # suggestions = get_word_suggestions(subtitle_text)
        # suggestions_str = "  |  ".join(suggestions) if suggestions else ""
        # cv2.rectangle(frame, (0, h_frame - 105), (w_frame, h_frame - 55), (40, 40, 40), -1)
        # cv2.putText(frame, suggestions_str,
        #             (10, h_frame - 75), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 0), 2)

        # Ukrainian translation bar (above suggestions)
        # cv2.rectangle(frame, (0, h_frame - 150), (w_frame, h_frame - 105), (20, 20, 20), -1)
        # frame = put_unicode_text(frame, ukrainian_text,
        #                          (10, h_frame - 145), font_size=28, color=(100, 255, 100))

        # Ukrainian translation bar (above original subtitles)
        cv2.rectangle(frame, (0, h_frame - 80), (w_frame, h_frame - 50), (20, 20, 20), -1)
        frame = put_unicode_text(frame, ukrainian_text,
                                 (10, h_frame - 80), font_size=28, color=(100, 255, 100))
